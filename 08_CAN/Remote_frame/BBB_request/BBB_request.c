...
void can0_transmit_obj_Remote_Frames_read(struct can_device_data *data, u8 msg_num){
    ...
    if1cmd &=~ (1u << 23); // CAN_IFxCMD_WR_RD: Read, Msg obj -> IF1
...
void Reset_msg_obj(struct can_device_data *data, u8 rd_wr, u8 msg_num, u8 msg_handler){
    ...
    iowrite32(0, data->base + CAN_IF1DATA); 
    iowrite32(0, data->base + CAN_IF1DATB); 
...
int dma_param_set(struct can_device_data* data, int ch, u8 byte_num, dma_addr_t dma_dst_addr){
    ...
    //dma_addr_t phys = dma_map_single(data->dev, dma_dst_addr, 4, DMA_TO_DEVICE);
...
#if (PRINT_DEBUG)
    printk("es = 0x%x, Int0ID = 0x%x, intpnd_x = 0x%x\n", es, Int0ID, intpnd_x);
#endif
    if (Int0ID == 0x8000){
        /* Int0ID = 0x8000 */
        if ((es&(1u << 3u))) {
        ...
        else if ((es&(1u << 4u))){ // RxOk -> Read new data
...
#else
#if (PRINT_DEBUG)
            printk("Received data[%d]:0x%x, 0x%x, count_many = %d\n", SIZE_REQUESTED, ioread32(data->base + CAN_IF1DATA)
                                                    , ioread32(data->base + CAN_IF1DATB), data->count_many);
#endif
            Dma_read(data); // enable DMA on DMA side
            can0_DMAactive_IF1(data); // enable DMA on CAN side
#if (PRINT_DEBUG)
            printk("data->buffer[0]:0x%x 0x%x 0x%x 0x%x\n", data->buffer[0]
                                                          , data->buffer[1]
                                                          , data->buffer[2]
                                                          , data->buffer[3]);
#endif
...
static void re_request_irq_work(struct work_struct *work)
{
    ...
    while (!atomic_read(&data->should_stop)){
        ...
        can0_transmit_obj_Remote_Frames(data, ID_CAN0, SIZE_REQUESTED, DMA_OBJ_MSG_NUM, 1);
        ...
        printk("Sent Remote frame with len expected: %d\n", SIZE_REQUESTED);
